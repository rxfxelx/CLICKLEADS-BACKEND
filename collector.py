# collector.py — browser singleton (sem corrida de event loop)
...
def _ensure_browser():
    """Garante uma única instância de Playwright/Browser (thread-safe)."""
    global _PW, _BROWSER
    if _BROWSER:
        return _BROWSER
    with _INIT_LOCK:
...
def _page_fast_and_fallback(query_str: str, start: int, limite: int, click_limit: int) -> List[str]:
    ...
    browser = _ensure_browser()  # usa singleton
    ...
    ctx.route("**/*", lambda route:
        route.abort() if route.request.resource_type in {"image","font","stylesheet","media"} else route.continue_()
    )
...
def collect_batch(nicho: str, state: Dict, limit: int) -> tuple[list[str], Dict, bool]:
    ...
    """
    # garante browser inicializado no main thread ANTES de abrir threads
    _ensure_browser()
...
# -------- APIs de alto nível usadas pelo server --------
...
def collect_numbers_info(nicho: str, local: str, limite: int = 50) -> tuple[list[str], bool]:
    ...
    for cidade in cities:
        ...
        while len(nums) < limite:
            batch, state, exhausted = collect_batch(nicho, state, rem)
            nums.extend([t for t in batch if t not in set(nums)])
